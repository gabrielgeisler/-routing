...
def ListDeposito():
    ...
    if params.get("id") == None:
        ...
        campos = ['Nº', 'Nome Depósito', 'Latitude', 'Longitude', 'Excluir', 'Alterar']
        for col, campo_nome in zip(colms, campos):
            col.write(campo_nome)

        for x, item in enumerate(ClienteController.SelecionarTodosDeposito()):
            col1, col2, col3, col4, col5, col6 = st.columns((1, 2, 1, 2, 1, 1))
            col1.write(item.id)
            col2.write(item.nome)
            col3.write(item.latitude)
            col4.write(item.longitude)
            ...
            if on_click_excluir:
                ClienteController.ExcluirDeposito(item.id)
